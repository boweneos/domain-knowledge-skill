...
**Shipped end-to-end. Current version: 0.4.2.** Four phases merged to `main` and tagged:
...
| `v0.4.2` (patch) | **MSG + XLSM parser support.** `.xlsm` (Excel macro-enabled) registered to the existing openpyxl-backed parser. New `src/dks/parsers/msg.py` parses Outlook `.msg` files via the `extract-msg` library: subject becomes a heading; From/To/Cc/Date become a single text block; body splits into paragraph-level blocks. All items share a `DocxLocator` with subject as section (no new locator type). Real-world surfaced when ingesting a 64-doc Neos corpus that mixed Outlook emails and macro-enabled spreadsheets. |

**Current version: 0.4.2.** 222 tests passing, 4 skipped (presidio absent in dev venv), mypy strict + ruff clean.
